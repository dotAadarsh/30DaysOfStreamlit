...
def load_model(x,y):
    ...
    d_trin = xgboost.DMatrix(X_train, label=y_train)
    ...
    model = xgboost.train(params, d_trin, 10, evals=[(d_test, "test")], verbose_eval= 100, early_stopping_rounds=20)
    return model
...
explaiener = shap.TreeExplainer(model)
